...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Initialize TopOn SDK (appID/appKey from TopOn dashboard)
    // MHAdSDK initialization is handled automatically by ATMHAdSDKInitAdapter
    NSError *error = nil;
    ...
    // Request ATT and location permissions with delay
        ...
        if (@available(iOS 14, *)) {
            [ATTrackingManager requestTrackingAuthorizationWithCompletionHandler:^(ATTrackingManagerAuthorizationStatus status) {
                if (status == ATTrackingManagerAuthorizationStatusAuthorized) {
                    NSLog(@"ATT: User authorized IDFA");
                } else {
                    NSLog(@"ATT: User denied IDFA");
                }
            }];
        } else {
            NSLog(@"ATT: iOS version too low to request IDFA");
        }
    });
